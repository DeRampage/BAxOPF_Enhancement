 ...
 *
 * Exslrle:
 * [_heli] call TF47_slingload_fnc_addACEActions
 */
...
if (_heliModel in TF47_customHooks) exitWith {
    ...
    //["model.p3d",[main, forward, aft]],
    {
        private _position = if (_x isNotEqualTo []) then {
            _action = [format ["TF47_slingload_attachCargo%1", _forEachIndex], _displayName, _icon, _statement, _condition, {}, _forEachIndex, _x, _distance] call ace_interact_menu_fnc_createAction;
...
if (_slingLoadPosition isNotEqualTo [0, 0, 0]) exitWith {
    _action = ["TF47_slingload_attachCargo0", _displayName, _icon, _statement, _condition, {}, CARGOHOOKINDEX_MAIN, _slingLoadPosition, _distance] call ace_interact_menu_fnc_createAction;
    [_heli, 0, [], _action] call ace_interact_menu_fnc_addActionToObject;
