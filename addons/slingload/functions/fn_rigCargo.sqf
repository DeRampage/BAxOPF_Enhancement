 ...
 *
 * example:
 * [_cargo, _unit] call TF47_slingload_fnc_rigCargo
 * [cursorObject, player] call TF47_slingload_fnc_rigCargo
 */

params ["_cargo", "_unit", ["_liftPoints", [], [[]]]];

if (_liftPoints isEqualTo []) then {
    _liftPoints = _cargo getVariable ["TF47_slingload_liftPoints", []];
};
if (_liftPoints isEqualTo []) then {
    _liftPoints = [_cargo] call TF47_slingload_fnc_getCargoLiftPoints;
};
...
_apexFitting setVariable ["TF47_slingload_cargo4Fitting", _cargo, true];
_apexFitting setVariable ["TF47_slingload_points4Fitting", _liftPoints, true];

_ropes = _ropes + (_cargo getVariable ["TF47_slingload_ropes4Cargo", []]) - [objNull];
_cargo setVariable ["TF47_slingload_ropes4Cargo", _ropes - [objNull], true];

[{
    [_this, ACE_Player] call TF47_slingload_fnc_pickUpFitting;
