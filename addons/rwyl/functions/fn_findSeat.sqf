...
if (isNull TF47_rwyl_vehicle) then {
    if _notInZeus then {
        ...
        private _end = (_start vectorAdd (getCameraViewDirection _unit vectorMultiply TF47_HopVehicleRange));
...
if (_hopVehicle) then {
    _sn = _sn select {
        ([TF47_rwyl_vehicle, _x] call TF47_rwyl_fnc_getProxyPosition) distance (TF47_rwyl_vehicle worldToModel getPos _unit) < TF47_HopVehicleRange
    };
};
TF47_rwyl_vehicle_distance = (_unit distance TF47_rwyl_vehicle) + TF47_HopVehicleRange;

// filter out blacklisted non-functional proxies
//_sn = _sn - getArray (configOf TF47_rwyl_vehicle >> "RWYL_proxyExcludeList");
_sn = _sn - (TF47_rwyl_vehicle getVariable ["RWYL_proxyExcludeList", []]);
{
    _sn pushBackUnique _x;
} forEach (TF47_rwyl_vehicle getVariable ["RWYL_proxyIncludeList", []]);
...
private _boundingTop = boundingBoxReal TF47_rwyl_vehicle # 1 # 2;
private _sp = _sn apply {[TF47_rwyl_vehicle, _x] call TF47_rwyl_fnc_getProxyPosition};
...
[{
    ...
    TF47_rwyl_proxy = nil;
    if (
        !TF47_rwyl_pfh_running
        || { // Hop vehicle target is too far
            TF47_rwyl_vehicle_distance > 0 && {_unit distance TF47_rwyl_vehicle > TF47_rwyl_vehicle_distance}
        }
    ) exitWith {
        [_pfID] call CBA_fnc_removePerFrameHandler;
        TF47_rwyl_vehicle_distance = -1;
    };
    ...
    if TF47_ShowAllSeats then {
        { // forEach _sn
            ...
            if (_forEachIndex != _indexClosest) then {
                ...
                drawIcon3D [_icon, TF47_OtherSeatsColour, TF47_rwyl_vehicle modelToWorldVisual (_sp select _forEachIndex), 0.8, 0.8, 0, _text];
            };
        } forEach _sn;
    };
    ...
    drawIcon3D [_icon, TF47_SelectedSeatColour, TF47_rwyl_vehicle modelToWorldVisual (_sp select _indexClosest), 1, 1, 0, _text];
